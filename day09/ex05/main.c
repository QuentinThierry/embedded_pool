...
volatile uint16_t g_value_cpt = 0;

void	display_complex_digits()
{
	uint16_t val = g_value_cpt;
	bool	begin = false;

	if (val / 1000 != 0)
	{
		display_num(val / 1000, DIGIT0);
		val %= 1000;
		begin = true;
	}
	if (val / 100 != 0 || begin)
	{
		display_num(val / 100, DIGIT1);
		val %= 100;
		begin = true;
	}
	if (val / 10 != 0 || begin)
	{
		begin = true;
		display_num(val / 10, DIGIT2);
		val %= 10;
	}
	display_num(val, DIGIT3);
}
